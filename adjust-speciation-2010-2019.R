  ...
  # Load blanks
  load("data/blanks-sum-2010-2019.RData")
  ...
  ##
  # function to pad fips
  fipspad <- function(x, l1) {
    str_pad(x, l1, "left", pad = "0")
  }
  ...
  for(j in 1 : length(cons)) {
    ...
    for(i in years) {
      ...
      # fix length of state, county, site
      x <- mutate(x, State.Code = fipspad(State.Code, 2),
                  County.Code = fipspad(County.Code, 3),
    ...
    cn <- gsub("\\.", "", gsub(" ", "", cons[j]))
    n1 <- paste0("data/y2010-2019/Krall_speciation-", cn, "-chdate.csv")
  ...
  blankbdl <- function(consU, bdl = F) {
    ...
    if(bdl) {
      ...
      # ** Should not be any missing for NH4, SO4, NO3, OC, EC **
      x <- mutate(x, Uncertainty = ifelse(is.na(Sample.Measurement) & !(Parameter.Name %in% bigs), 4 * geomean, Uncertainty),
  ...
  csndat <- adjustCSN()
  write.csv(csndat, file = "data/csndat2010-2019.csv", row.names = F)
  
  csndat <- adjustCSN(bdl = T)
  write.csv(csndat, file = "data/csndat2010-2019-bdl.csv", row.names = F)
